...
const MENU = [
  ...
  { to: "/attendance", label: "출석부", icon: "📅", end: false },
  { to: "/members", label: "멤버", icon: "👥", end: false },
  { to: "/settlement", label: "정산", icon: "💰", end: false },
  { to: "/plan", label: "통독 일정", icon: "📖", end: false },
  ...
  if (!isAdmin) {
    return (
      <div className="flex min-h-screen flex-col items-center justify-center gap-4 px-5 text-center">
        ...
        <h1 className="text-lg font-bold">운영자 권한이 필요합니다</h1>
        <p className="max-w-sm text-sm text-base-text/60">
          이 콘솔은 묵상대학 운영진 전용입니다. 계정({session.user.email})에 운영자 권한이 없어요.
    ...
    <div className="flex min-h-screen">
      ...
      <aside className="fixed inset-y-0 left-0 z-40 hidden w-56 flex-col bg-sidebar text-white/80 md:flex">
        <div className="px-5 py-6 leading-tight">
          <div className="text-[11px] font-medium tracking-wide text-white/50">운영자 콘솔</div>
